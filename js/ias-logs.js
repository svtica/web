...
desc[4] = ["NAS-IP-Address", "The IP address of the NAS originating the request"];
desc[5] = ["NAS-Port", "The physical port number of the NAS originating the request"];
desc[6] = ["Service-Type", "The type of service that the user has requested"];
desc[7] = ["Framed-Protocol", "The protocol to be used"];
desc[8] = ["Framed-IP-Address", "The framed IP address to be configured for the user."];
desc[9] = ["Framed-IP-Netmask", "The IP netmask to be configured for the user"];
desc[10] = ["Framed-Routing", "The routing method to be used by the user.", "\t", "11", "Filter-ID", "The name of the filter list for the user requesting authentication.", "\t", "12", "Framed-MTU", "The maximum transmission unit (MTU) to be configured for the user."];
desc[12] = ["Framed-MTU", ""];
desc[13] = ["Framed-Compression", "The compression protocol to be used."];
desc[14] = ["Login-IP-Host", "The IP address of the host to which the user should be connected."];
desc[15] = ["Login-Service", "The service that connects the user to the login host."];
desc[16] = ["Login-TCP-Port", "The TCP port to which the user is to be connected."];
desc[18] = ["Reply-Message", "The message displayed to the user when an authentication request is accepted."];
desc[19] = ["Callback-Number", "The callback phone number."];
desc[20] = ["Callback-ID", "The name of a location to be called by the access server when performing callback."];
desc[22] = ["Framed-Route", "The routing information that is configured on the access client."];
desc[23] = ["Framed-IPX-Network", "The Internetwork Packet Exchange (IPX) network number to be configured on the NAS for the user."];
desc[25] = ["Class", "The attribute sent to the client in an Access-Accept packet"];
desc[26] = ["Vendor-Specific", "The attribute that is used to support proprietary NAS features."];
desc[27] = ["Session-Timeout", "The length of time (in seconds) before a session is terminated."];
desc[28] = ["Idle-Timeout", "The length of idle time (in seconds) before a session is terminated."];
desc[29] = ["Termination-Action", "The action that the NAS is to take when service is completed."];
desc[30] = ["Called-Station-ID", "The phone number that is dialed by the user."];
desc[31] = ["Calling-Station-ID", "The phone number from which the call originated."];
desc[32] = ["NAS-Identifier", "The string that identifies the NAS originating the request."];
desc[34] = ["Login-LAT-Service", "The host with which the user is to be connected by Local Area Transport (LAT)."];
desc[35] = ["Login-LAT-Node", "The node with which the user is to be connected by LAT."];
desc[36] = ["Login-LAT-Group", "The LAT group codes for which the user is authorized."];
desc[37] = ["Framed-AppleTalk-Link", "The AppleTalk network number for the serial link to the user (this is used only when the user is a router)."];
desc[38] = ["Framed-AppleTalk-Network", "The AppleTalk network number that the NAS must query for existence in order to allocate the user AppleTalk node."];
desc[39] = ["Framed-AppleTalk-Zone", "The AppleTalk default zone for the user."];
desc[40] = ["Acct-Status-Type", "The number that specifies whether an accounting packet starts or stops a bridging, routing, or Terminal Services session."];
desc[41] = ["Acct-Delay-Time", "The length of time (in seconds) for which the NAS has been sending the same accounting packet."];
desc[42] = ["Acct-Input-Octets", "The number of octets received by NPS during the session."];
desc[43] = ["Acct-Output-Octets", "The number of octets sent by NPS during the session."];
desc[44] = ["Acct-Session-ID", "The unique numeric string that identifies the server session."];
desc[45] = ["Acct-Authentic", "The number that specifies which server has authenticated an incoming call."];
desc[46] = ["Acct-Session-Time", "The length of time (in seconds) for which the session has been active."];
desc[47] = ["Acct-Input-Packets", "The number of packets received by NPS during the session."];
desc[48] = ["Acct-Output-Packets", "The number of packets sent by NPS during the session."];
desc[49] = ["Acct-Terminate-Cause", "The reason that a connection was terminated by NPS."];
desc[50] = ["Acct-Multi-SSN-ID", "The unique numeric string that identifies the multilink session."];
desc[51] = ["Acct-Link-Count", "The number of links in a multilink session."];
desc[55] = ["Event-Timestamp", "The date and time that this event occurred on the NAS."];
desc[61] = ["NAS-Port-Type", "The type of physical port that is used by the NAS originating the request."];
desc[62] = ["Port-Limit", "The maximum number of ports that the NAS provides to the user."];
desc[63] = ["Login-LAT-Port", "The port with which the user is connected by LAT."];
desc[64] = ["Tunnel-Type", "The tunneling protocols to be used."];
desc[65] = ["Tunnel-Medium-Type", "The transport medium to use when creating a tunnel for protocols. For example, L2TP packets can be sent over multiple link layers."];
...
desc[75] = ["Password-Retry", "The number of times a user can try to be authenticated before the NAS terminates the connection."];
desc[76] = ["Prompt", "A number that indicates to the NAS whether or not it should (Prompt=1) or should not (Prompt=0) echo the user response as it is typed."];
desc[77] = ["Connect-Info", "Information that is used by the NAS to specify the type of connection made. Typical information includes connection speed and data encoding protocols."];
desc[78] = ["Configuration-Token", "The type of user profile to be used (sent from a RADIUS proxy server to a RADIUS client) in an Access-Accept packet."];
desc[81] = ["Tunnel-Pvt-Group-ID", "The group ID for a specific tunneled session."];
desc[82] = ["Tunnel-Assignment-ID", "The tunnel to which a session is to be assigned."];
desc[83] = ["Tunnel-Preference", "A number that indicates the preference of the tunnel type, as indicated by the Tunnel-Type attribute when multiple tunnel types are supported by the NAS."];
desc[85] = ["Acct-Interim-Interval", "The length of interval (in seconds) between each interim update sent by the NAS."];
desc[4108] = ["Client-IP-Address", "The IP address of the RADIUS client."];
desc[4116] = ["NAS-Manufacturer", "The manufacturer of the NAS."];
desc[4121] = ["MS-CHAP-Error", "The error data that describes a Microsoft Challenge Handshake Authentication Protocol (MS-CHAP) transaction."];
desc[4127] = ["Authentication-Type", "The authentication scheme that is used to verify the user."];
desc[4128] = ["Client-Friendly-Name", "The friendly name for the RADIUS client."];
desc[4129] = ["SAM-Account-Name", "The user account name in the Security Accounts Manager (SAM) database."];
desc[4130] = ["Fully-Qualified-User-Name", "The user name in canonical format."];
desc[4132] = ["EAP-Friendly-Name", "The friendly name that is used with Extensible Authentication Protocol (EAP)."];
...
valuedesc[4136] = {"1": "Accept-Request",
                   "2": "Access-Accept",
		   "3": "Access-Reject",
...
function parseNPS(in_box) {
   ...
   for (index in lines){
	 ...
	 try { line += desc[attribute][0] + ": " + valuedesc[attribute][value] + "\n"; }
	 catch(e) {
	     //describe attribute only
	     try { line += desc[attribute][0] + ": " + value + "\n"; }
	     //no descriptions availabe
	     catch(e) { line += attribute + ": " + value +"\n"; }
	 }
